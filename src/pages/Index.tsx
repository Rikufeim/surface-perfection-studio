...
import { Paintbrush, PaintRoller, Home, Phone, Mail, MapPin, Menu, X, CheckCircle, ArrowRight, Instagram, Facebook, Linkedin, Layers, Palette, Maximize2, MessageCircle } from 'lucide-react';
...
const referencesData = [{
  id: 1,
  src: portaatEnnen,
  title: "Portaikko ennen remonttia"
}, {
  id: 2,
  src: portaatValmis,
  title: "Portaikko remontin jälkeen"
}, {
  id: 3,
  src: keittioEnnen,
  title: "Keittiö ennen remonttia"
}, {
  id: 4,
  src: keittioValmis,
  title: "Keittiö remontin jälkeen"
}];
  ...
  return <div className="fixed bottom-6 right-6 z-[100] flex flex-col items-end gap-4" style={{ position: 'fixed' }}>
      {isOpen && <div className="flex flex-col gap-3 animate-in fade-in slide-in-from-bottom-8 duration-300 mb-2">
           <a href="https://wa.me/358458466055" target="_blank" rel="noopener noreferrer" className="flex items-center justify-end gap-3 group">
             ...
             <div className="w-10 h-10 rounded-full flex items-center justify-center text-white shadow-lg transition-transform hover:scale-110" style={buttonStyle}>
                <WhatsAppIcon size={20} className="text-white" />
             </div>
           </a>

           <a href="mailto:info@js-laatupinta.fi" className="flex items-center justify-end gap-3 group">
             <span className="bg-white text-neutral-900 px-3 py-1.5 rounded-lg shadow-lg font-bold text-sm opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
               Lähetä sähköpostia
             </span>
             <div className="w-10 h-10 rounded-full flex items-center justify-center text-white shadow-lg transition-transform hover:scale-110" style={buttonStyle}>
           ...
           <a href="tel:+358458466055" className="flex items-center justify-end gap-3 group">
             <span className="bg-white text-neutral-900 px-3 py-1.5 rounded-lg shadow-lg font-bold text-sm opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
               Soita
             </span>
             <div className="w-10 h-10 rounded-full flex items-center justify-center text-white shadow-lg transition-transform hover:scale-110" style={buttonStyle}>
      ...
      <button onClick={() => setIsOpen(!isOpen)} className="w-14 h-14 rounded-full flex items-center justify-center transition-all transform hover:scale-105 z-50" style={buttonStyle}>
  ...
  return <nav className={`absolute top-0 left-0 right-0 z-50 px-6 py-4`}>
      <div className="flex items-center justify-between w-full max-w-7xl mx-auto">
        ...
        <div className="hidden md:flex items-center gap-6 p-2">
          {navLinks.map(link => <button key={link.id} onClick={() => handleNavClick(link.id)} className={`text-sm font-semibold uppercase tracking-wide cursor-pointer transition-colors duration-200 px-4 py-1 ${activePage === link.id ? 'text-fuchsia-400' : 'text-gray-300 hover:text-fuchsia-400'}`}>
              {link.label}
            </button>)}
      ...
      {isOpen && <div className="absolute top-full mt-2 left-0 right-0 mx-4 border border-neutral-700 rounded-xl p-4 flex flex-col gap-4 shadow-xl md:hidden bg-black/90 backdrop-blur-sm animate-in fade-in slide-in-from-top-2 z-50">
          {navLinks.map(link => <button key={link.id} onClick={() => handleNavClick(link.id)} className={`text-left font-bold py-3 px-4 hover:bg-white/5 rounded-lg transition-all border-b border-white/5 last:border-0 ${activePage === link.id ? 'text-fuchsia-500' : 'text-gray-300 hover:text-fuchsia-400'}`}>
              {link.label}
            </button>)}
        </div>}
    </nav>;
    ...
    <div className="max-w-7xl mx-auto px-6 py-24 relative z-10 space-y-32">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        ...
        <div className="relative group order-1 lg:order-1">
          <div className="relative rounded-3xl overflow-hidden aspect-[4/3]">
            <img src="https://images.unsplash.com/photo-1589939705384-5185137a7f0f?q=80&w=1200&auto=format&fit=crop" alt="Sisämaalausta" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        ...
        <div className="relative group">
          <div className="relative rounded-3xl overflow-hidden aspect-[4/3]">
            <img src="https://images.unsplash.com/photo-1615873968403-89e068629265?q=80&w=1200&auto=format&fit=crop" alt="Tapetointia" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
          </div>
        </div>
      </div>
...
const HomeReferencesPreview = ({
  ...
  navigateTo: (id: string) => void;
       ...
       <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {referencesData.slice(0, 3).map(ref => <div key={ref.id} className="group relative aspect-[4/3] rounded-xl overflow-hidden bg-black/60 border border-white/5">
...
const ServicesPage = ({
  ...
  return <div className="w-full pb-10">
      <section className="min-h-screen flex flex-col items-start justify-start antialiased relative overflow-hidden w-full">
        ...
        <div className="p-6 max-w-7xl mx-auto relative z-20 w-full pt-32 md:pt-40">
           ...
           <div className="flex flex-col gap-32 pb-24">
              {detailedServices.map((service, index) => <div key={index} className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                  ...
                  <div className={`${index % 2 === 1 ? 'lg:order-2' : ''}`}>
                    ...
                    <ul className="space-y-3">
                      {service.features.map((feature, fIdx) => <li key={fIdx} className="flex items-start gap-3 text-neutral-300">
...
const ReferencesPage = () => {
  ...
  return <div className="pt-32 pb-24 min-h-screen bg-black" style={{
    ...
    filter: "brightness(1.05)"
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {referencesData.map(ref => <div key={ref.id} onClick={() => setSelectedImage(ref)} className="group relative aspect-[4/3] rounded-xl overflow-hidden cursor-pointer border-2 border-transparent hover:border-fuchsia-500/50 transition-all duration-300">
              <img src={ref.src} alt={ref.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
    ...
    filter: "brightness(1.05)"
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
          ...
          <div>
            ...
            <div className="mt-12">
              ...
              <div className="flex gap-4">
                {[Instagram, Facebook, Linkedin].map((Icon, i) => <a key={i} href="#" className="w-10 h-10 rounded-full border border-fuchsia-500/30 flex items-center justify-center text-neutral-400 hover:bg-fuchsia-600 hover:text-white hover:border-fuchsia-600 transition-all">
        ...
        <div className="mt-16 w-full h-64 md:h-80 rounded-xl overflow-hidden grayscale hover:grayscale-0 transition-all duration-700 border border-white/10">
          <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d1976.2415162431417!2d24.9654166!3d60.2941666!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x469207b7b7b7b7b7%3A0x123456789abcdef!2sSantaradantie%207%2C%2001380%20Vantaa!5e0!3m2!1sfi!2sfi!4v1620000000000!5m2!1sfi!2sfi" width="100%" height="100%" style={{
          border: 0
        }} allowFullScreen loading="lazy" title="Sijainti kartalla"></iframe>
        </div>
      </div>
    </div>;
};
